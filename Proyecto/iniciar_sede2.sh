...
# Función para checar componentes
check_component() {
    name="$1"
    pid=$2
    log=$3
...
DEV_LOG=$(mktemp)
java -cp "$CP" Gestor_carga.ActorClient_ZMQ ${GC_IP}:${PUB_PORT} 10.43.103.49:${GA_PORT} DEVOLUCION > "$DEV_LOG" 2>&1 &
DEV_PID=$!
...
REN_LOG=$(mktemp)
java -cp "$CP" Gestor_carga.ActorClient_ZMQ ${GC_IP}:${PUB_PORT} 10.43.103.49:${GA_PORT} RENOVACION > "$REN_LOG" 2>&1 &
REN_PID=$!
...
PRES_LOG=$(mktemp)
java -cp "$CP" Gestor_carga.ActorPrestamo_ZMQ ${GC_IP}:${PUB_PORT} 10.43.103.49:${GA_PORT} > "$PRES_LOG" 2>&1 &
PRES_PID=$!
